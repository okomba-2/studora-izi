...
import { GraduationCap, Eye, EyeOff, Mail, Lock, Loader2, Database, Key, RefreshCw, AlertCircle } from 'lucide-react';
...
export default function LoginView({ onNavigate, onLoginSuccess }: LoginViewProps) {
  ...
  const hasCustomDb = typeof window !== 'undefined' && !!localStorage.getItem('studora_supabase_url');

  const handleResetCustomDb = () => {
    playClickSound();
    localStorage.removeItem('studora_supabase_url');
    localStorage.removeItem('studora_supabase_anon_key');
    toast.show('Base personnalisée désactivée. Retour au mode local démo...', 'success');
    setTimeout(() => {
  ...
  const handleQuickAdminLogin = async (email: string) => {
    setValue('email', email);
    setValue('password', 'Divin500@');
    setIsLoading(true);
    playClickSound();
    try {
      await authService.signIn(email, 'Divin500@');
      playSuccessSound();
      toast.show('Connexion Administrateur réussie !', 'success');
      onLoginSuccess();
    } catch (err: any) {
      playFailureSound();
      toast.show(err.message || 'La connexion a échoué.', 'warning');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] px-4 py-16 relative overflow-hidden">
      ...
      >
        ...
        {/* Dynamic Support and Debugging panel for Administrators */}
        {isMock ? (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.3 }}
            className="mt-6 p-5 bg-amber-500/5 border border-amber-500/10 rounded-2xl text-left backdrop-blur-md space-y-4"
          >
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1 space-y-1">
                <h4 className="text-xs font-bold text-amber-950 uppercase tracking-wider">
                  Accès Rapide Administration (Mode Simulation)
                </h4>
                <p className="text-[11px] text-slate-500 leading-relaxed">
                  Supabase n'est pas encore configuré globalement. L'authentification tourne en mode simulation locale sécurisée. Cliquez ci-dessous pour vous connecter directement.
                </p>
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-slate-100 flex flex-col gap-2">
              <div className="flex flex-col sm:flex-row gap-2">
                <button
                  type="button"
                  id="btn-admin-login-500"
                  onClick={() => handleQuickAdminLogin('okomba500@gmail.com')}
                  className="flex-1 px-3 py-2 bg-amber-600 hover:bg-amber-700 active:scale-[0.98] transition-all text-white text-xs font-bold rounded-lg shadow-sm cursor-pointer text-center flex items-center justify-center space-x-1"
                >
                  <span>okomba500@gmail.com (Admin)</span>
                </button>
                <button
                  type="button"
                  id="btn-admin-login-contact"
                  onClick={() => handleQuickAdminLogin('okombacontact@gmail.com')}
                  className="flex-1 px-3 py-2 bg-slate-800 hover:bg-slate-900 active:scale-[0.98] transition-all text-white text-xs font-bold rounded-lg shadow-sm cursor-pointer text-center flex items-center justify-center space-x-1"
                >
                  <span>okombacontact@gmail.com</span>
                </button>
              </div>
              <p className="text-[10px] text-slate-500 text-center italic">
                Mot de passe par défaut : <span className="font-mono font-bold text-amber-700 bg-amber-100/50 px-1.5 py-0.5 rounded">Divin500@</span>
              </p>
            </div>
          </motion.div>
        ) : (
          hasCustomDb && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.3 }}
              className="mt-6 p-4 bg-rose-50 border border-rose-100 rounded-2xl text-left space-y-3"
            >
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-rose-800 leading-relaxed mt-1">
                    Vous êtes connecté à une base de données Cloud. Si vous n'avez pas encore créé l'utilisateur admin ou si vous souhaitez retourner au mode démo :
                  </p>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-2 bg-rose-600 hover:bg-rose-700 active:scale-[0.98] transition-all text-white text-xs font-bold rounded-lg flex items-center justify-center space-x-2 shadow-xs cursor-pointer"
              >
                <RefreshCw className="w-3.5 h-3.5" />
                <span>Bascule vers la simulation locale (Mode Démo)</span>
              </button>
            </motion.div>
          )
        )}
